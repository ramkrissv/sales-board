...
function buildPDFContent(stats: { pipeline: string; activeDeals: number; wonDeals: number; accounts: number; ee: number; en: number; nn: number }) {
  ...
  return `<!DOCTYPE html><html><head>
    ...
    <ul>
      ...
      <li><strong>10-Tab Workshop</strong> — Overview → Intake → <strong>Whiteboard</strong> (4-zone workspace: Sticky Wall, Notes &amp; Outline, Sketch Pad, Media Gallery + AI Copilot with 8 action types. PPTX/DOCX/PDF server-side parsing. Section fullscreen with rich text editor. S3 storage for audio/images/files. Voice recording with AI transcription.) → <strong>Assess</strong> (Auto-Score from Whiteboard + Dynamic Dimensions from Whiteboard — AI creates new dimensions from discovery topics. Smart chat with auto-ranking.) → Use Cases (upload docs → AI extracts + prioritizes) → Scope → Findings (5 data-driven deliverable assets) → Proposal → Builder → Settings. Stage Summary convergence bars on every tab. Fullscreen toggle.</li>
