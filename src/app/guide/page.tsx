...
function buildPDFContent(stats: { pipeline: string; activeDeals: number; wonDeals: number; accounts: number; ee: number; en: number; nn: number }) {
  ...
  return `<!DOCTYPE html><html><head>
    ...
    <ul>
      ...
      <li><strong>10-Tab Workshop</strong> — Overview → Intake → <strong>Whiteboard</strong> (freeform canvas with draggable stickies, zones, Canvas+Structured dual view, voice recording, camera OCR for sticky note photos, AI copilot sidebar) → Assess (smart chat with auto-ranking, score accept/reject, whiteboard context) → Use Cases (inline edit name/sponsor/problem) → Scope (delete scope items) → Findings (5 data-driven deliverable assets: Assessment Report, Roadmap, Architecture Deep Dive, Executive Briefing, Gap Analysis — all code-rendered infographics) → Proposal → Builder (edit/delete levels, workstreams, dimensions) → Settings. <strong>Fullscreen toggle</strong> on every tab.</li>
