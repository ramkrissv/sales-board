...
function buildPDFContent(stats: { pipeline: string; activeDeals: number; wonDeals: number; accounts: number; ee: number; en: number; nn: number }) {
  ...
  return `<!DOCTYPE html><html><head>
    ...
    <ul>
      ...
      <li><strong>9-Tab Workshop</strong> — Overview (SVG readiness dial, 5 KPIs, per-level expandable dimension cards, maturity distribution chart, top gaps ranked, workstream impact, assessment intelligence auto-insights, consistency check) → Intake (4 sections, custom Qs, AI autofill, doc upload) → Assess (0-4 scoring, AI chat sidebar, Deep Discovery) → Use Cases (tower grouping, matrix, AI pilot recommend) → Scope (AI generation, execution models: Pod Squad, Managed Capacity, Outcome-Based, AI-Powered, Hybrid) → Findings (executive hero banner with gap severity, maturity distribution, strengths/weaknesses, deep level analysis with dimension drill-down, workstream impact, AI narrative, Strategic Recommendations Engine with 4 categories — Quick Wins, Foundation, Strategic, Governance — plus AI deep generation with impact/effort/KPIs/dependencies per recommendation, investment &amp; phasing roadmap, readiness checklist) → Proposal (gradient hero + 5 KPIs, investment stacked bar chart, per-module editable cards with traceability, delivery model mix, phase distribution, proposal completeness gate) → Builder (dynamic framework editor) → Settings.</li>
