...
async function processSignal(text: string, senderName: string, source: string) {
  ...
  // Auto-link to existing deals (prevent duplicates) — DON'T auto-create
  ...
  if (Notification) {
    await Notification.create({
      ...
      title: `${ai?.matchedDealId ? '🔵 Signal Linked' : '🟡 New Lead'}: ${ai?.customerName || senderName}`,
      ...
      metadata: { source, senderName, matchedDealId: ai?.matchedDealId, matchedDealName: ai?.matchedDealName,
        ...
        status: ai?.matchedDealId ? 'linked' : 'pending_acceptance' },
    });
  }
