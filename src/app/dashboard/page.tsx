  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Performance across {opportunities.length} opportunities</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={kpi.label} className="p-4 rounded-xl bg-[#111127] border border-[#2a2a4a]">
            <div className="text-[11px] text-slate-500 uppercase tracking-wider mb-1">{kpi.label}</div>
      ...
      {/* Sales Funnel */}
      <div className="p-5 rounded-xl bg-[#111127] border border-[#2a2a4a]">
        <div className="text-sm font-medium text-white mb-4">Sales Funnel</div>
          ...
          {funnelData.map((stage, i) => {
            ...
            return (
              <div key={stage.status} className="flex items-center gap-3">
                <div className="w-24 text-xs text-slate-400 text-right">{stage.status}</div>
                <div className="flex-1 h-8 bg-[#1a1a35] rounded-lg overflow-hidden">
                  <div
                ...
                <div className="w-20 text-xs text-slate-500 text-right">${(stage.tcv/1000).toFixed(0)}k</div>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* By Owner */}
        <div className="p-5 rounded-xl bg-[#111127] border border-[#2a2a4a]">
          <div className="text-sm font-medium text-white mb-4">By Owner</div>
          <div className="space-y-3">
            {topOwners.map(([owner, data]) => (
              <div key={owner} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-slate-500">{data.count} deals</span>
                  {data.tcv > 0 && <span className="text-xs text-slate-400">${(data.tcv/1000).toFixed(0)}k</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        {/* By Industry */}
        <div className="p-5 rounded-xl bg-[#111127] border border-[#2a2a4a]">
          <div className="text-sm font-medium text-white mb-4">By Industry</div>
          <div className="space-y-3">
